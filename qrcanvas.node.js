...
function drawRound(contextData) {
  const { cellSize, context, options, isDark, colorDark } = contextData;
...
function drawLiquid(contextData) {
  const { cellSize, context, isDark, colorDark, options } = contextData;
  const radius = options.effect.value * cellSize / 2;
  drawCells(contextData, ({ i, j, x, y }) => {
...
function drawImage(contextData) {
  const { context, cellSize, count, colorDark, options } = contextData;
  drawCells(contextData, ({ i, j, x, y }) => {
    context.fillStyle = colorDark;
    let fillSize = 0.25;
...
function drawImageFore(contextData) {
  const { cellSize, size, mask, options } = contextData;
...
class QRCanvas {
  constructor(options) {
    this.isDark = (i, j) => {
      ...
      return i >= 0 && i < count && j >= 0 && j < count && this.shouldTransclude(i + j * count) && qr.isDark(i, j);
    };
  ...
  setOptions(userOptions) {
    const options = Object.assign({}, defaultOptions(), userOptions, {
      effect: Object.assign({}, userOptions.effect)
    });
    if (userOptions.logo && (userOptions.logo.image || userOptions.logo.text)) {
      const logo = Object.assign({}, defaultLogoOptions(), userOptions.logo);
  ...
  initLogo() {
    ...
    const normalize = () => {
      ...
      const kl = Math.min((iHeight * cellSize - 2 * logo.margin) / height, (iWidth * cellSize - 2 * logo.margin) / width, 1);
    ...
    if (logo.image) {
      ...
      height = 100;
      const font = [logo.fontStyle, `${height}px`, logo.fontFamily].filter(Boolean).join(' ');
      width = measureText(logo.text, font).width;
      normalize();
      const ctx = logo.canvas.getContext('2d');
      ctx.font = [logo.fontStyle, `${logo.height}px`, logo.fontFamily].filter(Boolean).join(' ');
  ...
  getForegroundArea(contextData, data) {
    ...
    if (effect.foreground) {
      return effect.foreground(Object.assign({}, contextData, {
        mask: () => {
          const canvas = getCanvas(size);
          const defaultEffect = getEffect();
          this.drawCells(Object.assign({}, contextData, {
            canvas,
            context: canvas.getContext('2d'),
            colorDark: 'black',
